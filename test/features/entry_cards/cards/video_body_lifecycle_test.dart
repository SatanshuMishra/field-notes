...
const String _busyHint = 'Tap to try again';

Finder _labelledIn(int card, String label) => find.descendant(
      of: cardAt(card),
      matching: find.bySemanticsLabel(label),
    );

Completer<void> releasableVideoGate() {
...
FakeMediaResolver _resolverOfClips(Map<String, File> clips) {
  final FakeMediaResolver resolver = FakeMediaResolver();
  clips.forEach((String mediaId, File file) {
    resolver.set(
      mediaId,
...
void main() {
  group('VideoBody under cap pressure', () {
    ...
    testWidgets('a permanently denying registry answers a tap with an '
        'announced refusal', (WidgetTester tester) async {
  ...
  group('VideoBody transport gating', () {
    testWidgets('a second tap in the claiming frame never commands a loading '
        'player', (WidgetTester tester) async {
      final LruVideoSlots slots = slotsWithCapOf(1);
      final Completer<void> gate = releasableVideoGate();
      final List<FakeEntryVideoPlayer> built = <FakeEntryVideoPlayer>[];
      ...
      expect(built, hasLength(1));
      expect(tapEnabled(tester, inCard(1, videoPlayToggleKey)), isTrue);
    ...
    testWidgets('a claim denied by a pinned holder is announced instead of '
        'doing nothing', (WidgetTester tester) async {
      ...
      expect(
        tester
            .getSemantics(_labelledIn(1, _busyNotice))
            .getSemanticsData()
            .flagsCollection
            .isLiveRegion,
        isTrue,
      ...
      expect(
        tester
            .getSemantics(inCard(1, videoPlayToggleKey))
            .getSemanticsData()
            .hint,
        _busyHint,
      );
    ...
    testWidgets('a freed slot clears the announced denial', (
      WidgetTester tester,
    ) async {
      final LruVideoSlots slots = slotsWithCapOf(1);
      final List<FakeEntryVideoPlayer> built = <FakeEntryVideoPlayer>[];
  ...
  group('VideoBody resolver changes', () {
    ...
    testWidgets('a new resolver identity re-prepares on the slot it already '
        'holds', (WidgetTester tester) async {
      final LruVideoSlots slots = slotsWithCapOf(1);
      final List<FakeEntryVideoPlayer> built = <FakeEntryVideoPlayer>[];
      final File file = videoFixtureFile();
  ...
  group('VideoBody media identity changes', () {
    testWidgets('a same key card given a new mediaId plays the new clip', (
      WidgetTester tester,
    ) async {
      final LruVideoSlots slots = slotsWithCapOf(2);
      final List<FakeEntryVideoPlayer> built = <FakeEntryVideoPlayer>[];
      final File first = videoFixtureFile(prefix: 'video_first');
      final File second = videoFixtureFile(prefix: 'video_second');
      final FakeMediaResolver resolver = _resolverOfClips(<String, File>{
        'vid-a': first,
        'vid-b': second,
      });
      final EntryVideoPlayerFactory factory = videoFactoryInto(built);
      ...
      await tester.pumpWidget(
        _columnOfMediaIds(
          ...
          playerFactory: factory,
          slots: slots,
          mediaIds: <String>['vid-a', 'vid-a'],
        ),
      );
      await tester.pump();

      expect(built, hasLength(2));
      expect(built[0].loadCalls, <String>[first.path]);
      ...
      expect(built, hasLength(3));
      expect(built[2].loadCalls, <String>[second.path]);
      expect(built[0].disposeCalls, 1);
      expect(surfaceMounted(tester, 0), isTrue);
      expect(find.byType(CorruptMediaPlaceholder), findsNothing);
    });

    testWidgets('a re-preparing card keeps its slot instead of losing it to a '
        'waiting sibling', (WidgetTester tester) async {
      final LruVideoSlots slots = slotsWithCapOf(2);
      final List<FakeEntryVideoPlayer> built = <FakeEntryVideoPlayer>[];
      final File first = videoFixtureFile(prefix: 'video_first');
      final File second = videoFixtureFile(prefix: 'video_second');
      ...
      await tester.pumpWidget(
        _columnOfMediaIds(
          ...
          playerFactory: factory,
          slots: slots,
          mediaIds: <String>['vid-a', 'vid-a', 'vid-a'],
        ),
      );
      await tester.pump();
      ...
      await tester.pumpWidget(
        _columnOfMediaIds(
          ...
          playerFactory: factory,
          slots: slots,
          mediaIds: <String>['vid-b', 'vid-a', 'vid-a'],
        ),
      );
      await tester.pump();
      await tester.pump();
    ...
    testWidgets('a re-prepare that resolves to nothing hands its slot to a '
        'waiting sibling', (WidgetTester tester) async {
      final LruVideoSlots slots = slotsWithCapOf(1);
      final List<FakeEntryVideoPlayer> built = <FakeEntryVideoPlayer>[];
      final File first = videoFixtureFile(prefix: 'video_first');
      ...
      await tester.pumpWidget(
        _columnOfMediaIds(
          ...
          playerFactory: factory,
          slots: slots,
          mediaIds: <String>['vid-gone', 'vid-a'],
        ),
      );
      await tester.pump();
      await tester.pump();
      ...
      expect(built, hasLength(2));
      expect(built[1].loadCalls, <String>[first.path]);
      expect(surfaceMounted(tester, 1), isTrue);
      expect(readyControlsEnabled(tester, 1), isTrue);
    });
  });
}
