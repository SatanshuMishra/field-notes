...
## Active Decisions
- decisions/2026-07-21-capture-flow-root-cause-and-fix.md — capture flows: note/voice save-hang (Riverpod retry-limbo, provider_container.dart:948) + video start()/_ready deadlock FIXED; "no prompt" = ad-hoc-signing/TCC env limit
...
## Threads
- journal-app-design — paused — capture flows fixed + 4 code-review touch-ups + real-UI macOS app test (3/3) shipped in PR #32 (merged, origin/main c2fbefd); next = local run + DB-connectivity check. See sessions/2026-07-21-07.
...
- RUN wf_888cd869-d28 (human-gated) ENDED PARTIAL at 30/31: engine built + PR'd capture-photo #27, settings-screen #28, capture-voice #29, capture-video #30; I validated each locally (FOREGROUND) and the USER merged each (gh pr merge is agent-blocked — see decisions/2026-07-21-gh-merge-hook-blocked-human-merges.md). shell-nav-integration parked at branch-composition (2 parent checkpoint refs missing because their engine force-pushes were classifier-blocked); I recreated the refs non-destructively so a fresh relaunch composes it. RELAUNCH MUST BE FRESH-CONTEXT. Do NOT resume run id wf_888cd869-d28 — launch the verbatim 2026-07-11-03 human-gated block fresh (logicalRunId 5385f00d reuses the checkpoint refs).
- run.json is a compact append-log (now ~8 lines: base + 2 park deltas + built checkpoints); it folds to 31. On the next relaunch ONLY shell-nav-integration rebuilds fresh; the 30 merged fast-skip via the live `gh pr list --state merged` reconcile.
...
- .claude/ledger/sessions/2026-07-21-07-journal-app-design.md — latest (touch-ups + real-UI app test 3/3 macOS; PR #32 merged)
