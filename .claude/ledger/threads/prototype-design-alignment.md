...
completion_criteria:
  ...
  - "[x] The Today screen, right rail, nav rail, flower set, mood picker and capture surfaces are human-confirmed against the prototype on macOS hardware — MET 2026-08-01 on cfa04c7"
  - OQ-3 and OQ-6 are answered or explicitly closed as out of scope
next_step: Re-land D4. Cut a branch from `origin/main` (`cfa04c7`), cherry-pick `e29362d` from `msp-cluster-d/d1-rail-container`, run fullValidationCmd expecting **904**, open a PR with `--base main`. NEVER PR that branch into main; it would delete the slice.
branch: `origin/main` `cfa04c7` (slice #87, D1 #88, ledger #92, D3 #93 landed). D2 and D4 still stranded on `msp-cluster-d/d1-rail-container` (tip `46705d3`).
---

## Status
**18 of the parent spec's 39 MSPs are on main** (A1-A5, B1-B4, C1-C7, D1, D3). The Cluster D slice is landed (`docs/specs/2026-07-29-prototype-alignment-cluster-d.md`, 617 lines). D3 was recovered onto main this session as #93. D4 (`e29362d`) and D2 (`46705d3`) remain stranded on D1's branch. D2 shipped chrome only and still owes a product ruling. Mitosis stays excluded for the whole remaining spec.
...
## Next Step
Re-land D4 from a branch cut off `cfa04c7`, then resolve D2. Full ordered list in sessions/2026-08-01-02-prototype-design-alignment.md's "Deferred + open".

## Open Risks
- **Never PR `msp-cluster-d/d1-rail-container` into main.** It predates the slice's merge, so the diff deletes `docs/specs/2026-07-29-prototype-alignment-cluster-d.md` at -617 lines plus later ledger files. Re-land D2/D4 by cherry-pick onto a branch cut from current main.
- **D2 owes a ruling only the user can give.** Its tap-to-Calendar — the ONE genuinely new behaviour in Cluster D and its stated acceptance criterion — is unimplemented because every route needs `lib/app/shell/**`, which the slice's §0 and §6.2 close. Options: declare the widening, promote the shell destination through Riverpod, or drop the criterion. Its re-land needs a fresh PR anyway, which incidentally fixes #91's mis-titled subject (it never reached main).
- **D2's worktree head is `1aa0f15`, not the `46705d3` that merged.** Reconcile which is authoritative before re-landing.
- **Baseline is now 903, not 904** (decisions/2026-08-01-d3-chooser-test-deletion-is-correct.md). D4 therefore predicts **904**, not the 905 recorded when the baseline was 904. Always predict the count from the diff BEFORE running `fullValidationCmd`.
- **`today_screen_test.dart` is shared by D3 and D4.** D3 landed first, so D4's rebase must PRESERVE the `_allCaptureRoutes()` helper D3 added and its switch of the registry override off `CaptureRouteRegistry.empty` (rows only render for registered types).
- **The ledger's branch/merge state has been wrong SEVEN times.** On resume, always re-check `gh pr view` and `git log origin/main` before acting.
...
- **For one long command, background it from the main thread** rather than wrapping it in a subagent — a dispatched agent returned empty twice this session and its detached shell was swept with teardown.
...
- **Orphan branches and three stale stashes still owe a confirmed batch removal.** This session removed only the d1 and d3 cluster-D worktrees; d2 and d4 are kept, and no branch was deleted.
...
## Recent Sessions
- sessions/2026-08-01-02-prototype-design-alignment.md — **D3 re-landed on main (#93); 903 ruled correct; integration flows green; visual pass PASSED. Next actions live here**
- sessions/2026-08-01-01-prototype-design-alignment.md — Cluster D slice + D1 landed; D2/D3/D4 stranded on D1's branch
- sessions/2026-07-29-02- / 2026-07-29-01- / 2026-07-28-07- through -01-prototype-design-alignment.md
