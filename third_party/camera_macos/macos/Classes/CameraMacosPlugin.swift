...
public class CameraMacosPlugin: NSObject, FlutterPlugin, FlutterTexture, AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate, AVAssetWriterDelegate, AVCaptureFileOutputRecordingDelegate {
            ...
            if granted {
                ...
                do {
                    ...
                    if self.useMovieFileOutput {
                        ...
                        if self.captureSession.canAddOutput(previewVideoOutput) {
                            previewVideoOutput.videoSettings = [kCVPixelBufferPixelFormatTypeKey as String: kCVPixelFormatType_32BGRA]
                            previewVideoOutput.alwaysDiscardsLateVideoFrames = true
                            previewVideoOutput.setSampleBufferDelegate(self, queue: .main)
                            self.captureSession.addOutput(previewVideoOutput)
            ...
            if(!isRecording) {
                ...
                if self.useMovieFileOutput {
                    ...
                    if let maxVideoDuration = arguments["maxVideoDuration"] as? Double, maxVideoDuration > 0 {
                        movieOutput.maxRecordedDuration = CMTimeMakeWithSeconds(Double(maxVideoDuration), preferredTimescale: 600)
                    }
    ...
    public func fileOutput(_ output: AVCaptureFileOutput, didFinishRecordingTo outputFileURL: URL, from connections: [AVCaptureConnection], error: Error?) {
        ...
        if !ok {
            let detail: String = error?.localizedDescription ?? "unknown recording error"
            let payload = FlutterError(code: "MOVIE_FILE_OUTPUT_FAIL", message: "Recording finished unexpectedly at \(outputFileURL.absoluteURL.path) - \(detail)", details: nil).toFlutterResult
            DispatchQueue.main.async {
                self.outputChannel?.invokeMethod("onVideoRecordingFinished", arguments: payload)
            }
        }
    }
