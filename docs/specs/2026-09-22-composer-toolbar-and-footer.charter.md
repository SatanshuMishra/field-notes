# Charter — Composer Toolbar and Footer (phase 5)

Binding on every Worker in the 2026-09-22 composer phase 5 wave. Read all of it before your first Step.
...
- State is Riverpod 3 with `riverpod_generator`; storage is drift. Generated `*.g.dart` files are
  committed.
- The authority for this wave is `docs/specs/2026-09-22-composer-toolbar-and-footer.md`. Read it before
  your first Step. Above it sit `docs/specs/2026-09-22-note-composer-inline-photos.md` and then
  `docs/specs/2026-09-20-note-editor-and-scrapbook-photos.md`. Where they disagree, the phase 5 document
  wins, then the inline-photo document. Nothing under `docs/specs/archive/` is binding; do not read it.
- The note is one string and the editor is one text field (C3). A photo is the line
  `![caption](photo/<ref> "side size")`, and every edit to one goes through the functions in
  `lib/features/notes/photos/photo_line_edits.dart`. Never write that Markdown by hand.
- The editor draws photos in place from `lib/features/capture/text/editor/in_place_photo_editor.dart`,
  which positions figures in its own render object at layout time. Read it before you draw anything
  else into that canvas.
