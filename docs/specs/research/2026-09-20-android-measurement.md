...
| 5 | `feedScrollFrame` | One dragged 60 pt scroll step over the real `TodayScreen` sliver feed, seeded with 40 note entries long enough to truncate. Since the Today feed moved to compact log cards, this measures those cards, each showing a lead, a snippet and a thumbnail; a result recorded before that change measured the old full-length preview cards and is not comparable. |
