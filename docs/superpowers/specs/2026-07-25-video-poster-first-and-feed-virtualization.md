...
**Amendment (2026-07-26, ratified by the spec owner):** Phase 2 executes with `shrinkWrap: true` on the replacement `ListView` — the Option B ruling, preserving the modal's shrink-to-fit sizing. In this bounded position (`Flexible` under `ConstrainedBox(maxHeight: 520)`) a shrink-wrapping viewport still virtualizes: `RenderShrinkWrappingViewport` takes its build-everything path only when the incoming main-axis extent is infinite (`packages/flutter/lib/src/rendering/viewport.dart:2153-2158`); otherwise it lays out lazily against the finite extent and clamps its height via `constraints.constrainHeight(_shrinkWrapExtent)` (`:2116-2119`), with no scroll-time height wobble in either content-size case. The blanket `shrinkWrap` rejection under Phase 3's Forbidden shortcuts and under Rejected alternatives is hereby SCOPED to unbounded positions; it remains fully in force for the Phase 3 Today feed, which has no bounded ancestor. Ruling recorded in `.claude/ledger/decisions/2026-07-26-day-detail-shrinkwrap-option-b.md`.
...
- **`shrinkWrap: true` on a `ListView`/`CustomScrollView` is forbidden as a substitute for a real sliver rewrite.** In this feed's UNBOUNDED position, `shrinkWrap` sizes the scrollable to its children's total main-axis extent — the viewport becomes as tall as its content, the sliver protocol's laziness is defeated, and every child still builds (`RenderShrinkWrappingViewport`'s infinite-main-axis branch, `viewport.dart:2153-2158`). See https://github.com/flutter/flutter/issues/26072 and https://github.com/flutter/flutter/pull/181092. This is a rewrite that changes nothing about eager building. Scope note (2026-07-26 amendment): this reasoning is specific to unbounded positions and does not transfer to the bounded Day-detail panel — see the Phase 2 amendment.
...
- **`shrinkWrap: true` as a virtualization shortcut.** Rejected for UNBOUNDED positions (the Phase 3 Today feed): there it defeats the sliver protocol's laziness by making the viewport's main-axis extent match its content, so every child still builds regardless of visibility. See https://github.com/flutter/flutter/issues/26072 and https://github.com/flutter/flutter/pull/181092. Scoped by the 2026-07-26 amendment: ratified for the bounded Day-detail panel (Phase 2), where the finite incoming constraint keeps the viewport lazy (`viewport.dart:2116-2119`, `:2153-2172`).
